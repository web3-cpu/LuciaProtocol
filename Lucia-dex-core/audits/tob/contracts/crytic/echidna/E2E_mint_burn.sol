...
import '../../../../../contracts/LuciaPool.sol';
...
contract E2E_mint_burn {
    ...
    constructor() public {
        ...
        lucia = new SetupLucia(token0, token1);

        minter = new LuciaMinter(token0, token1);
        swapper = new LuciaSwapper(token0, token1);
    ...
    function check_mint_invariants(
        int24 _tickLower,
        int24 _tickUpper,
        LuciaMinter.MinterStats memory bfre,
        LuciaMinter.MinterStats memory aftr
    ) internal {
        (, int24 currentTick, , , , , ) = pool.slot0();
    ...
    function _init(uint128 _seed) internal {
        ...
        // deploy the pool
        //
        lucia.createPool(poolParams.fee, poolParams.startPrice);
        pool = lucia.pool();
    ...
    function test_mint(uint128 _amount) public {
        ...
        (LuciaMinter.MinterStats memory bfre, LuciaMinter.MinterStats memory aftr) =
    ...
    function test_burn_partial(uint128 _amount) public {
        ...
        try minter.doBurn(pos.tickLower, pos.tickUpper, burnAmount) returns (
            LuciaMinter.MinterStats memory bfre_burn,
            LuciaMinter.MinterStats memory aftr_burn
        ) {
            bfre = bfre_burn;
            aftr = aftr_burn;
        } catch {
