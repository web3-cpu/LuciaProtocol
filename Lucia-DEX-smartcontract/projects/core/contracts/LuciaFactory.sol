...
/// @title Canonical Lucia factory
/// @notice Deploys Lucia pools and manages ownership and control over pool protocol fees
contract LuciaFactory is ILuciaFactory {
    /// @inheritdoc ILuciaFactory
    ...
    function createPool(address tokenA, address tokenB, uint24 fee) external override returns (address pool) {
        ...
        pool = ILuciaPoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        ILuciaPool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return ILuciaPool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        ILuciaPool(pool).setLmPool(lmPool);
    }
}
