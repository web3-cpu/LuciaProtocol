...
contract LuciaPoolDeployer is ILuciaPoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @param factory The contract address of the LuicaSwap factory
    ...
    function deploy(
        ...
        pool = address(new LuciaPool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}
