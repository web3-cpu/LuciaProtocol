...
import './interfaces/ILuciaPoolDeployer.sol';
import './interfaces/ILuciaFactory.sol';
...
import '@luciaswap/lm-pool/contracts/interfaces/ILuciaLmPool.sol';

contract LuciaPool is ILuciaPool {
    ...
    /// @inheritdoc ILuciaPoolImmutables
    ...
    modifier onlyFactoryOrFactoryOwner() {
        require(msg.sender == factory || msg.sender == ILuciaFactory(factory).owner());
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = ILuciaPoolDeployer(msg.sender).parameters();
    ...
    /// @inheritdoc ILuciaPoolDerivedState
    ...
    function mint(
        ...
        ILuciaMintCallback(msg.sender).luciaMintCallback(amount0, amount1, data);
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            ILuciaSwapCallback(msg.sender).luciaSwapCallback(amount0, amount1, data);
    ...
    function flash(address recipient, uint256 amount0, uint256 amount1, bytes calldata data) external override lock {
        ...
        ILuciaFlashCallback(msg.sender).luciaFlashCallback(fee0, fee1, data);
    ...
    /// @inheritdoc ILuciaPoolOwnerActions
