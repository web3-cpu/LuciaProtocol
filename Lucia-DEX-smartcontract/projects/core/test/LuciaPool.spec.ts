...
import { TestLuciaSwapPay } from '../typechain-types/contracts/test/TestLuciaSwapPay'
...
import { TestLuciaReentrantCallee } from '../typechain-types/contracts/test/TestLuciaReentrantCallee'
...
describe('LuciaPool', () => {
  ...
  let factory: LuciaFactory
  let pool: MockTimeLuciaPool

  let swapTarget: TestLuciaCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeLuciaPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestLuciaReentrantCallee')
      ).deploy()) as TestLuciaReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestLuciaSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestLuciaSwapPay')
      underpay = (await underpayFactory.deploy()) as TestLuciaSwapPay
