...
describe('LuciaFactory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: LuciaPoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as LuciaPoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as LuciaFactory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [LuciaFactory, LuciaPoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('LuciaPool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('LuciaPool')
