...
import { LuciaPoolSwapTest } from '../typechain-types/contracts/test/LuciaPoolSwapTest'
...
describe('LuciaPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('LuciaPoolSwapTest')
            const tester = (await testerFactory.deploy()) as LuciaPoolSwapTest
