import { abi as ILuciaPoolABI } from '@luciaswap/core/artifacts/contracts/interfaces/ILuciaPool.sol/ILuciaPool.json'
import { Fixture } from 'ethereum-waffle'
import { BigNumber, constants, ContractTransaction, Wallet } from 'ethers'
import { ethers, waffle } from 'hardhat'
import { ILuciaPool, IWETH9, MockTimeSwapRouter, TestERC20 } from '../typechain-types'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [ILuciaPool, ILuciaPool, ILuciaPool]
    ...
    const pools = poolAddresses?.map((poolAddress: any) => new ethers.Contract(poolAddress, ILuciaPoolABI, wallet)) as [
      ILuciaPool,
      ILuciaPool,
      ILuciaPool
    ]
  ...
  let pools: [ILuciaPool, ILuciaPool, ILuciaPool]
  ...
  describe('#exactInput', () => {
    ...
    it.skip('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestLuciaCallee')
