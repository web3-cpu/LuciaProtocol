...
const artifacts: { [name: string]: ContractJson } = {
  ...
  LuciaInterfaceMulticall: require('../artifacts/contracts/lens/LuciaInterfaceMulticall.sol/LuciaInterfaceMulticall.json'),
...
async function main() {
  ...
  const luciaPoolDeployer_address = deployedContracts.LuciaPoolDeployer
  const luciaFactory_address = deployedContracts.LuciaFactory

  const SwapRouter = new ContractFactory(artifacts.SwapRouter.abi, artifacts.SwapRouter.bytecode, owner)
  const swapRouter = await SwapRouter.deploy(luciaPoolDeployer_address, luciaFactory_address, config.WNATIVE)

  // await tryVerify(swapRouter, [luciaPoolDeployer_address, luciaFactory_address, config.WNATIVE])
  ...
  const baseTokenUri = 'https://nft.luciaswap.com/v3/'
  console.log('nonfungibleTokenPositionDescriptor started')
  ...
  const nonfungiblePositionManager = await NonfungiblePositionManager.deploy(
    luciaPoolDeployer_address,
    luciaFactory_address,
    config.WNATIVE,
  ...
  const LuciaInterfaceMulticall = new ContractFactory(
    artifacts.LuciaInterfaceMulticall.abi,
    artifacts.LuciaInterfaceMulticall.bytecode,
    owner
  )

  const luciaInterfaceMulticall = await LuciaInterfaceMulticall.deploy()
  console.log('LuciaInterfaceMulticall', luciaInterfaceMulticall.address)

  // await tryVerify(luciaInterfaceMulticall)
  ...
  const quoterV2 = await QuoterV2.deploy(luciaPoolDeployer_address, luciaFactory_address, config.WNATIVE)
  console.log('QuoterV2', quoterV2.address)

  // await tryVerify(quoterV2, [luciaPoolDeployer_address, luciaFactory_address, config.WNATIVE])
  ...
  const contracts = {
    ...
    LuciaInterfaceMulticall: luciaInterfaceMulticall.address,
  }
