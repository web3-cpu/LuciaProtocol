...
import '@luciaswap/core/contracts/interfaces/callback/ILuciaFlashCallback.sol';
...
/// @notice An example contract using the LuciaSwap V3 flash function
contract PairFlash is ILuciaFlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `LuciaFlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        ILuciaPool pool = ILuciaPool(PoolAddress.computeAddress(deployer, poolKey));
