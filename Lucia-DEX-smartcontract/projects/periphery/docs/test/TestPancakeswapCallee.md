...
```solidity
function luciaSwapCallback(int256 amount0Delta, int256 amount1Delta, bytes data) external
```

Called to `msg.sender` after executing a swap via ILuciaPool#swap.
...
| data         | bytes  | Any data passed through by the caller via the ILuciaPoolActions#swap call                                                                                                               |
