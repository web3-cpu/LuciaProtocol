...
An example contract using the Lucia Swap flash function
...
```solidity
function luciaFlashCallback(uint256 fee0, uint256 fee1, bytes data) external
```
...
Calls the pools flash function with data needed in `LuciaFlashCallback`
