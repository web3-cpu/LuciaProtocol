...
import '@luciaswap/periphery/contracts/base/BlockTimestamp.sol';
import '@luciaswap/periphery/contracts/libraries/Path.sol';
import '@luciaswap/periphery/contracts/libraries/PoolAddress.sol';
import '@luciaswap/core/contracts/interfaces/ILuciaPool.sol';
import '@luciaswap/periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(ILuciaPool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (ILuciaPool pool) {
        pool = ILuciaPool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            ILuciaPool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = ILuciaPool(pool).slot0();
            }
