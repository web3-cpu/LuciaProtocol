...
```solidity
mapping(uint256 => struct MasterChef.PoolInfo) poolInfo
```

Info of each MC pool.

### userPositionInfos

```solidity
mapping(uint256 => struct MasterChef.UserPositionInfo) userPositionInfos
```
...
poolPid[token0][token1][fee] => pid

### poolAddressPid

```solidity
mapping(address => uint256) poolAddressPid
```

poolAddressPid[poolAddress] => pid
...
### latestPeriodLCIPerSecond

```solidity
uint256 latestPeriodLCIPerSecond
```
...
```solidity
uint256 LCIAmountBelongToMC
```

Record the LCI amount belong to MasterChef.
...
```solidity
event AddPool(uint256 pid, uint256 allocPoint, contract ILuciaPool pool, contract ILMPool lmPool)
```
...
```solidity
event NewUpkeepPeriod(uint256 periodNumber, uint256 startTime, uint256 endTime, uint256 LCIPerSecond, uint256 LCIAmount)
```

### UpdateUpkeepPeriod

```solidity
event UpdateUpkeepPeriod(uint256 periodNumber, uint256 oldEndTime, uint256 newEndTime, uint256 remainingLCI)
```
...
```solidity
constructor(contract IERC20 _LCI, contract INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) public
```
...
| ---------------------------- | ------------------------------------ | ------------------------------------------ |
| \_LCI                        | contract IERC20                      | The LCI token contract address.            |
...
Returns the LCI per second , period end time.
...
```solidity
function getLatestPeriodInfo(address _pool) public view returns (uint256 LCIPerSecond, uint256 endTime)
```

Returns the LCI per second , period end time. This is for liquidity mining pool.
...
View function for checking pending LCI rewards.

_The pending LCI amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest._
...
```solidity
function add(uint256 _allocPoint, contract ILuciaPool _pool, bool _withUpdate) external
```

Add a new pool. Can only be called by the owner.
One pool can only create one pool.
...
| \_pool       | contract ILuciaPool | Address of the pool.                          |
...
Update the given pool's LCI allocation point. Can only be called by the owner.
...
```solidity
function harvestOperation(struct MasterChef.UserPositionInfo positionInfo, uint256 _tokenId, address _to) internal returns (uint256 reward)
```
...
```solidity
function updateLiquidityOperation(struct MasterChef.UserPositionInfo positionInfo, uint256 _tokenId, uint256 _newMultiplier) internal
```
...
Transfer token from MasterChef .
...
| Name         | Type    | Description                               |
| ------------ | ------- | ----------------------------------------- |
| \_amount     | uint256 | The amount of LCI injected.               |
...
Update LCI reward for all the liquidity mining pool.
...
Update LCI reward for the liquidity mining pool.
...
| Name     | Type    | Description               |
| -------- | ------- | ------------------------- |
| \_to     | address | The LCI receiver address. |
| \_amount | uint256 | Transfer LCI amounts.     |
