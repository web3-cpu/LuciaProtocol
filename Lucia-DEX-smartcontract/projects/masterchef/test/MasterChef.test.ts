...
import LuciaPoolDeployerArtifact from "@luciaswap/core/artifacts/contracts/LuciaPoolDeployer.sol/LuciaPoolDeployer.json";
import LuciaFactoryArtifact from "@luciaswap/core/artifacts/contracts/LuciaFactory.sol/LuciaFactory.json";
// import LuciaFactoryOwnerArtifact from "@luciaswap/core/artifacts/contracts/LuciaFactoryOwner.sol/LuciaFactoryOwner.json";
import LuciaSwapRouterArtifact from "@luciaswap/periphery/artifacts/contracts/SwapRouter.sol/SwapRouter.json";
import NftDescriptorOffchainArtifact from "@luciaswap/periphery/artifacts/contracts/NonfungibleTokenPositionDescriptorOffChain.sol/NonfungibleTokenPositionDescriptorOffChain.json";
import NonfungiblePositionManagerArtifact from "@luciaswap/periphery/artifacts/contracts/NonfungiblePositionManager.sol/NonfungiblePositionManager.json";
import LuciaLmPoolDeployerArtifact from "@luciaswap/lm-pool/artifacts/contracts/LuciaLmPoolDeployer.sol/LuciaLmPoolDeployer.json";
import TestLiquidityAmountsArtifact from "@luciaswap/periphery/artifacts/contracts/test/LiquidityAmountsTest.sol/LiquidityAmountsTest.json";

import ERC20MockArtifact from "./ERC20Mock.json";
import LCITokenArtifact from "./LCIToken.json";
...
describe("MasterChefV3", function () {
  ...
  beforeEach(async function () {
    ...
    // Deploy factory
    const LuciaPoolDeployer = await ethers.getContractFactoryFromArtifact(LuciaPoolDeployerArtifact);
    const luciaPoolDeployer = await LuciaPoolDeployer.deploy();

    const LuciaFactory = await ethers.getContractFactoryFromArtifact(LuciaFactoryArtifact);
    const luciaFactory = await LuciaFactory.deploy(luciaPoolDeployer.address);

    await luciaPoolDeployer.setFactoryAddress(luciaFactory.address);

    const LuciaSwapRouter = await ethers.getContractFactoryFromArtifact(LuciaSwapRouterArtifact);
    const luciaSwapRouter = await LuciaSwapRouter.deploy(luciaPoolDeployer.address, luciaFactory.address, WETH9Address);
    ...
    // );
    // const baseTokenUri = "https://nft.panlciswap.com/v3/";
    // const nonfungibleTokenPositionDescriptor = await upgrades.deployProxy(NonfungibleTokenPositionDescriptor, [
    //   baseTokenUri,
    // ]);
    // await nonfungibleTokenPositionDescriptor.deployed();
    // TODO:
    await LuciaSwapRouter.deploy(luciaPoolDeployer.address, luciaFactory.address, WETH9Address);
    ...
    const nonfungiblePositionManager = await NonfungiblePositionManager.deploy(
      luciaPoolDeployer.address,
      luciaFactory.address,
      WETH9Address,
    ...
    // const LuciaFactoryOwner = await ethers.getContractFactoryFromArtifact(LuciaFactoryOwnerArtifact);
    // const luciaFactoryOwner = await LuciaFactoryOwner.deploy(luciaFactory.address);
    // await luciaFactory.setOwner(luciaFactoryOwner.address);

    // Prepare for master chef v3
    const LCIToken = await ethers.getContractFactoryFromArtifact(LCITokenArtifact);
    const lciToken = await LCIToken.deploy();

    const SyrupBar = await ethers.getContractFactoryFromArtifact(SyrupBarArtifact);
    const syrupBar = await SyrupBar.deploy(lciToken.address);
    ...
    await lciToken.transferOwnership(masterChef.address);
    ...
    const masterChefV2 = await MasterChefV2.deploy(masterChef.address, lciToken.address, 2, admin.address);
    ...
    const MasterChefV3 = await ethers.getContractFactory("MasterChef");
    const masterChefV3 = await MasterChefV3.deploy(lciToken.address, nonfungiblePositionManager.address, WETH9Address);
    ...
    const LuciaLmPoolDeployer = await ethers.getContractFactoryFromArtifact(LuciaLmPoolDeployerArtifact);
    const luciaLmPoolDeployer = await LuciaLmPoolDeployer.deploy(
      masterChefV3.address
      // luciaFactoryOwner.address
    );
    // await luciaFactoryOwner.setLmPoolDeployer(luciaLmPoolDeployer.address);
    await luciaFactory.setLmPoolDeployer(luciaLmPoolDeployer.address);
    await masterChefV3.setLMPoolDeployer(luciaLmPoolDeployer.address);
    ...
    await tokenA.connect(admin).approve(luciaSwapRouter.address, ethers.constants.MaxUint256);
    await tokenB.connect(admin).approve(luciaSwapRouter.address, ethers.constants.MaxUint256);
    await tokenC.connect(admin).approve(luciaSwapRouter.address, ethers.constants.MaxUint256);
    await tokenD.connect(admin).approve(luciaSwapRouter.address, ethers.constants.MaxUint256);
    ...
    // const lciFarmed = await lciToken.balanceOf(admin.address);
    // console.log(`${ethers.utils.formatUnits(lciFarmed)} LCI farmed`);
    await lciToken.approve(masterChefV3.address, ethers.constants.MaxUint256);
    await masterChefV3.setReceiver(admin.address);
    await masterChefV3.upkeep(ethers.utils.parseUnits(`${4 * 24 * 60 * 60}`), 24 * 60 * 60, true);
    // console.log(`lciPerSecond: ${ethers.utils.formatUnits((await masterChefV3.latestPeriodLCIPerSecond()).div(await masterChefV3.PRECISION()))}\n`);
    ...
    this.lciToken = lciToken;
    this.liquidityAmounts = liquidityAmounts;
    this.swapRouter = luciaSwapRouter;
  ...
  describe("Real world user flow", function () {
    context("when there are 2 users and 2 pools with no trading", function () {
      it("should executed successfully", async function () {
        ...
        lciUser1 = (await this.lciToken.balanceOf(user1.address)).add(await this.masterChefV3.pendingLCI(1));

        console.log("@5 ----------------------------------------");
        console.log(`user1: ${ethers.utils.formatUnits(lciUser1)}`);
        console.log("");
        ...
        lciUser1 = (await this.lciToken.balanceOf(user1.address))
          .add(await this.masterChefV3.pendingLCI(1))
          .add(await this.masterChefV3.pendingLCI(2));
        ...
        lciUser1 = (await this.lciToken.balanceOf(user1.address))
          .add(await this.masterChefV3.pendingLCI(1))
          .add(await this.masterChefV3.pendingLCI(2));
        lciUser2 = (await this.lciToken.balanceOf(user2.address)).add(await this.masterChefV3.pendingLCI(3));

        console.log("@7 ----------------------------------------");
        console.log(`user1: ${ethers.utils.formatUnits(lciUser1)}`);
        console.log(`user2: ${ethers.utils.formatUnits(lciUser2)}`);
        console.log("");
        ...
        lciUser1 = (await this.lciToken.balanceOf(user1.address))
          .add(await this.masterChefV3.pendingLCI(1))
          .add(await this.masterChefV3.pendingLCI(2));
        lciUser2 = (await this.lciToken.balanceOf(user2.address))
          .add(await this.masterChefV3.pendingLCI(3))
          .add(await this.masterChefV3.pendingLCI(4));
        ...
        lciUser1 = (await this.lciToken.balanceOf(user1.address))
          .add(await this.masterChefV3.pendingLCI(1))
          .add(await this.masterChefV3.pendingLCI(2))
          .add(await this.masterChefV3.pendingLCI(5));
        lciUser2 = (await this.lciToken.balanceOf(user2.address))
          .add(await this.masterChefV3.pendingLCI(3))
          .add(await this.masterChefV3.pendingLCI(4));
        ...
        lciUser2 = (await this.lciToken.balanceOf(user2.address))
          .add(await this.masterChefV3.pendingLCI(3))
          .add(await this.masterChefV3.pendingLCI(4))
          .add(await this.masterChefV3.pendingLCI(6));
        ...
        lciUser1 = (await this.lciToken.balanceOf(user1.address))
          ...
          .add(await this.masterChefV3.pendingLCI(2))
          .add(await this.masterChefV3.pendingLCI(5))
          .add(await this.masterChefV3.pendingLCI(7));
        lciUser2 = (await this.lciToken.balanceOf(user2.address))
          .add(await this.masterChefV3.pendingLCI(3))
          .add(await this.masterChefV3.pendingLCI(4))
          .add(await this.masterChefV3.pendingLCI(6));
        ...
        lciUser1 = (await this.lciToken.balanceOf(user1.address))
          ...
          .add(await this.masterChefV3.pendingLCI(5))
          .add(await this.masterChefV3.pendingLCI(7))
          .add(await this.masterChefV3.pendingLCI(8));
        lciUser2 = (await this.lciToken.balanceOf(user2.address))
          .add(await this.masterChefV3.pendingLCI(3))
          .add(await this.masterChefV3.pendingLCI(4))
          .add(await this.masterChefV3.pendingLCI(6));
        ...
        lciUser2 = (await this.lciToken.balanceOf(user2.address))
          ...
          .add(await this.masterChefV3.pendingLCI(4))
          .add(await this.masterChefV3.pendingLCI(6))
          .add(await this.masterChefV3.pendingLCI(9));
        ...
        lciUser1 = (await this.lciToken.balanceOf(user1.address))
          ...
          .add(await this.masterChefV3.pendingLCI(7))
          .add(await this.masterChefV3.pendingLCI(8))
          .add(await this.masterChefV3.pendingLCI(10));
        lciUser2 = (await this.lciToken.balanceOf(user2.address))
          .add(await this.masterChefV3.pendingLCI(3))
          .add(await this.masterChefV3.pendingLCI(4))
          .add(await this.masterChefV3.pendingLCI(6))
        ...
        lciUser2 = (await this.lciToken.balanceOf(user2.address))
          ...
          .add(await this.masterChefV3.pendingLCI(6))
          .add(await this.masterChefV3.pendingLCI(9))
          .add(await this.masterChefV3.pendingLCI(11));
        ...
        assert(lciUser1.sub(ethers.utils.parseUnits("57.68974359")).abs().lte(ethers.utils.parseUnits("0.1")));
        assert(lciUser2.sub(ethers.utils.parseUnits("105.3102564")).abs().lte(ethers.utils.parseUnits("0.1")));
      });
    });
    ...
    context("when there are 2 users and 1 pool with different range positions", function () {
      it("should executed successfully", async function () {
        ...
        lciUser1 = (await this.lciToken.balanceOf(user1.address)).add(await this.masterChefV3.pendingLCI(1));
        lciUser2 = (await this.lciToken.balanceOf(user2.address)).add(await this.masterChefV3.pendingLCI(2));
        ...
        assert(lciUser1.sub(ethers.utils.parseUnits("28.73260345")).abs().lte(ethers.utils.parseUnits("0.0000001")));
        assert(lciUser2.sub(ethers.utils.parseUnits("167.2673966")).abs().lte(ethers.utils.parseUnits("0.0000001")));
      });
    });
    ...
    context("when there are 2 users and 1 pool with different range positions", function () {
      it("should executed successfully", async function () {
        ...
        assert(lciUser1.sub(ethers.utils.parseUnits("73.99948783")).abs().lte(ethers.utils.parseUnits("0.0000001")));
        assert(lciUser2.sub(ethers.utils.parseUnits("122.0005122")).abs().lte(ethers.utils.parseUnits("0.0000001")));
      });
    });
  });
});
