...
contract MasterChef is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice Address of LCI contract.
    IERC20 public immutable LCI;
    ...
    uint256 public latestPeriodLCIPerSecond;
    ...
    /// @notice Record the LCI amount belong to MasterChefV3.
    uint256 public LCIAmountBelongToMC;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, ILuciaPool indexed v3Pool, ILMPool indexed lmPool);
    ...
    /// @param _LCI The LCI token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20 _LCI, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        LCI = _LCI;
        nonfungiblePositionManager = _nonfungiblePositionManager;
        WETH = _WETH;
    }

    /// @notice Returns the LCI per second , period end time.
    /// @param _pid The pool pid.
    /// @return LCIPerSecond LCI reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256 LCIPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            LCIPerSecond = (latestPeriodLCIPerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice Returns the LCI per second , period end time. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return LCIPerSecond LCI reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256 LCIPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            LCIPerSecond =
                (latestPeriodLCIPerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) /
                totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice View function for checking pending LCI rewards.
    /// @dev The pending LCI amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    /// @param _tokenId Token Id of NFT.
    /// @return reward Pending reward.
    function pendingLCI(uint256 _tokenId) external view returns (uint256 reward) {
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (LCI.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    function add(uint256 _allocPoint, ILuciaPool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    /// @notice Update the given pool's LCI allocation point. Can only be called by the owner.
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotLuciaNFT();
    ...
    /// @notice harvest LCI from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward LCI reward.
    ...
    function transferToken(address _token, address _to) internal {
        ...
        // Need to reduce LCIAmountBelongToMC.
        if (_token == address(LCI)) {
            unchecked {
                // In fact balance should always be greater than or equal to LCIAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balance >= LCIAmountBelongToMC) {
                    balance -= LCIAmountBelongToMC;
                } else {
                    // This should never happend.
                    LCIAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        // Need to reduce LCIAmountBelongToMC.
        if (token == address(LCI)) {
            unchecked {
                // In fact balance should always be greater than or equal to LCIAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balanceToken >= LCIAmountBelongToMC) {
                    balanceToken -= LCIAmountBelongToMC;
                } else {
                    // This should never happend.
                    LCIAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of LCI injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        // Transfer LCI token from receiver.
        LCI.safeTransferFrom(receiver, address(this), _amount);
        // Update LCIAmountBelongToMC
        unchecked {
            LCIAmountBelongToMC += _amount;
        }
        ...
        uint256 LCIPerSecond;
        uint256 LCIAmount = _amount;
        if (latestPeriodEndTime > currentTime) {
            uint256 remainingLCI = ((latestPeriodEndTime - currentTime) * latestPeriodLCIPerSecond) / PRECISION;
            emit UpdateUpkeepPeriod(latestPeriodNumber, latestPeriodEndTime, currentTime, remainingLCI);
            LCIAmount += remainingLCI;
        }
        LCIPerSecond = (LCIAmount * PRECISION) / duration;
        unchecked {
            latestPeriodNumber++;
            latestPeriodStartTime = currentTime + 1;
            latestPeriodEndTime = endTime;
            latestPeriodLCIPerSecond = LCIPerSecond;
        }
        emit NewUpkeepPeriod(latestPeriodNumber, currentTime + 1, endTime, LCIPerSecond, LCIAmount);
    }

    /// @notice Update LCI reward for all the liquidity mining pool.
    ...
    /// @notice Update LCI reward for the liquidity mining pool.
    ...
    /// @notice Safe Transfer LCI.
    /// @param _to The LCI receiver address.
    /// @param _amount Transfer LCI amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = LCI.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            // Update LCIAmountBelongToMC
            unchecked {
                if (LCIAmountBelongToMC >= _amount) {
                    LCIAmountBelongToMC -= _amount;
                } else {
                    LCIAmountBelongToMC = balance - _amount;
                }
            }
            LCI.safeTransfer(_to, _amount);
        }
    }
