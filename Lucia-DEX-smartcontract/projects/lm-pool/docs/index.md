...
```solidity
function getLatestPeriodInfo(address _v3Pool) external view returns (uint256 LCIPerSecond, uint256 endTime)
```
...
```solidity
contract ILuciaFactoryOwner factoryOwner
```
...
```solidity
constructor(address _masterChef, contract ILuciaFactoryOwner _factoryOwner) public
```

### deploy

```solidity
function deploy(contract ILuciaPool pool) external returns (contract ILuciaLmPool lmPool)
```
...
| ---- | ------------------- | ------------------------------------------ |
| pool | contract ILuciaPool | The contract address of the LuciaSwap pool |
