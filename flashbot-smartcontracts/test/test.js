...
describe("LuciaDodoFlashSwap contract", () => {
  ...
  beforeEach(async () => {
    ...
    const FlashSwap = await ethers.getContractFactory("LuciaDodoFlashSwap");
            ...
            for(j1 = 0; j1 < config.baseAssets.length; j1++) {
              ...
              for(j2 = 0; j2 < config.tokens.length; j2++) {
                ...
                for(j3 = 0; j3 < config.tokens.length; j3++) {
                  ...
                  if (token1.address == token3.address && token2.address == token3.address) continue;
                  const amtBack = await FLASHSWAP.estimateTriArbTrade(router1.address, router2.address, router3.address, token1.address, token2.address, token3.address, loanAmount);
                  //const amtBackHuman = ethers.utils.formatUnits(amtBack, dec);
                  if (amtBack.gt(0)) {

                    console.log(`--- Router: ${router1.dex} -> ${router2.dex} -> ${router3.dex} ---`);
                    console.log(`Swapping: ${token1.sym} -> ${token2.sym} -> ${token3.sym} -> ${token1.sym}`);
