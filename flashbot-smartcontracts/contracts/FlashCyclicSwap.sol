...
contract FlashCyclicSwap is Ownable {
    ...
    function estimateCyclicArbTrade(
        ...
        amtBack = _tokens[0] != _tokens[_tokens.length - 1]
            ? getAmountOutMin(
    ...
    function _triArbTrade(
        ...
        for (uint i = 0; i < _tokens.length - 1; i++) {
            if (_tokens[i] != _tokens[i + 1]) {
                initTokenBalances[i + 1] = IERC20(_tokens[i + 1]).balanceOf(address(this));
                _swap(_routers[i], _tokens[i], _tokens[i + 1], tradeableAmount);
                tokenBalances[i + 1] = IERC20(_tokens[i + 1]).balanceOf(address(this));
                tradeableAmount = tokenBalances[i + 1] - initTokenBalances[i + 1];
            }
        }
        if (_tokens[_tokens.length - 1] != _tokens[0]) {
            initTokenBalances[0] = IERC20(_tokens[0]).balanceOf(address(this));
            _swap(_routers[_tokens.length - 1], _tokens[_tokens.length - 1], _tokens[0], tradeableAmount);
