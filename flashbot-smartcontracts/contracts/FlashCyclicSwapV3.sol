...
contract FlashCyclicSwapV3 is Ownable {
    struct SearchVars {
        address[] routerPath;
        address[] tokenPath;
        uint8 dec;
    ...
    function searchArb(
        ...
        uint iend = (_start + _count) / _tokens.length;
        uint jend = (_start + _count) % _tokens.length;
        uint istart = _start / _tokens.length;
        uint jstart = _start % _tokens.length;

        if (istart > _routers.length - 1) istart = _routers.length - 1;
        if (iend > _routers.length) iend = _routers.length;
        if (jend == 0) jend = _tokens.length;

        uint[] memory arbRouterIds = new uint[](
            _limit == 0 ? (iend - istart) * (jend - jstart) : _limit
        );
        uint[] memory arbTokensIds = new uint[](
            _limit == 0 ? (iend - istart) * (jend - jstart) : _limit
        );
        ...
        for (uint i = istart; i < iend; i++) {
            vars.routerPath = _routers[i];
            for (uint j = jstart; j < jend; j++) {
                vars.tokenPath = _tokens[j];
                vars.dec = IERC20(vars.tokenPath[0]).decimals();
                vars.amount = 10 ** vars.dec / 10000;
                vars.amtBack = vars.amount;

                for (uint ii = 0; ii < vars.routerPath.length - 1; ii++) {
                    vars.amtBack = getAmountOutMin(
                        vars.routerPath[ii],
                        vars.tokenPath[ii],
                        vars.tokenPath[ii + 1],
                        vars.amtBack
                ...
                vars.amtBack = getAmountOutMin(
                    vars.routerPath[vars.routerPath.length - 1],
                    vars.tokenPath[vars.routerPath.length - 1],
                    vars.tokenPath[0],
                    vars.amtBack
                );
                if (vars.amtBack > vars.amount) {
                    arbRouterIds[vars.counter] = i + 1;
                    arbTokensIds[vars.counter] = j + 1;
                    vars.counter++;
                }
                if (vars.counter > arbRouterIds.length - 1) break;
            }
        }
        return (arbRouterIds, arbTokensIds, vars.counter);
    }
