...
contract LuciaStableSwapThreePool is Ownable, ReentrancyGuard {
    ...
    ILuciaStableSwapLP public token;
    ...
    function set_bnb_gas(uint256 _bnb_gas) external onlyOwner {
        require(
            _bnb_gas >= MIN_BNB_GAS && _bnb_gas <= MAX_BNB_GAS,
            "Illegal gas"
        );
        bnb_gas = _bnb_gas;
