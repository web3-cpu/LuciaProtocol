...
contract LuciaStableSwapTwoPoolInfo {
    uint256 public constant N_COINS = 2;
    uint256 public constant FEE_DENOMINATOR = 1e10;
    uint256 public constant PRECISION = 1e18;

    function token(address _swap) public view returns (IERC20) {
        return IERC20(ILuciaStableSwap(_swap).token());
    }
    ...
    function balances(
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            swapBalances[i] = ILuciaStableSwap(_swap).balances(i);
        }
    }
    ...
    function RATES(
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            swapRATES[i] = ILuciaStableSwap(_swap).RATES(i);
        }
    }
    ...
    function PRECISION_MUL(
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            swapPRECISION_MUL[i] = ILuciaStableSwap(_swap).PRECISION_MUL(i);
        }
    }
    ...
    function calc_coins_amount(
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            uint256 value = (ILuciaStableSwap(_swap).balances(i) * _amount) /
                total_supply;
            amounts[i] = value;
        }
        return amounts;
    ...
    function get_add_liquidity_mint_amount(
        ...
        ILuciaStableSwap swap = ILuciaStableSwap(_swap);
    ...
    function _xp(
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            result[i] =
                (result[i] * ILuciaStableSwap(_swap).balances(i)) /
                PRECISION;
        }
    }
