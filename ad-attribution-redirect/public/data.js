...
async function udata() {
  ...
  return {
    ...
    /*
    data: {
    redirectHash: '23a18e142185fc6f47d660b15d10b0eb6f0d377b7b0ed092370c7ae3625250f2',
    data: {
      isMetaMaskInstalled: false,
      os: 'Mac OSX (Version Unknown)|Unknown',
      touch: false,
...
async function redirect() {
  ...
  const path = window.redirectPath;
  const ip = window.redirectIp;
  if (!path || path === "") {
    setTimeout(function () {
  ...
  if (localStorage && localStorage.getItem("lid")) {
    req.lid = JSON.stringify(localStorage.getItem("lid"));
  }

  try {
    //fetch("https://api.clickinsights.xyz" + "/api/link/redirect/", {
    fetch(window.backendURL + "/api/link/redirect/", {
      // TODO
      method: "POST",
      headers: headers,
      ...
      .then((data) => {
        ...
        setTimeout(function () {
          localStorage.setItem("lid", data.lucia_user_hash);
          const target = data.target + "?lucia=" + srch;
...
function generateRandom32BitInt() {
  return Math.floor(Math.random() * 0xffffffff);
}

// Generate a 256-bit random integer in hexadecimal
function generateRandom256BitHex() {
  // Combine eight random 32-bit integers to form a 256-bit integer
  const intArray = [];
  for (let i = 0; i < 8; i++) {
    intArray.push(generateRandom32BitInt());
  }
  return intArray.map((num) => num.toString(16).padStart(8, "0")).join("");
}
