...
describe("SimpleSwap", function () {
  ...
  it("Should get token mapping addresses", async function () {
    const simpleSwapFactory = await ethers.getContractFactory("SimpleSwap");
    const simpleSwap = await simpleSwapFactory.deploy(SwapRouterAddress);
    await simpleSwap.deployed();
    const Shiba_address = await simpleSwap.getAddress('SHIB');
    const Rollbit_address = await simpleSwap.getAddress('RLB');
    const Maker_address = await simpleSwap.getAddress('MKR')
    expect(Shiba_address).to.be.a.properAddress;
    expect(Rollbit_address).to.be.a.properAddress;
    expect(Maker_address).to.be.a.properAddress;
  })
});