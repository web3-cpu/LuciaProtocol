 ...
 */
 ...
 const baseFormulas = [
   ...
   },
   {
     key: 'MULTIPLY',
     title: tf('formula.product'),
     render: ([a, b]) => (Number(a) * Number(b).toFixed(2)),
   },
   {
     key: 'PERCENTOFTOTAL',
     title: tf('formula.product'),
     render: ary => ary.reduce((a, b) => ((Number(a) * Number(((1 - b))))).toFixed(4)),
   },
   {
     key: 'SUBTRACT',
   ...
   {
     key: 'RATIO',
     title: tf('formula.ratio'),
     render: ary => ary.reduce((a, b) => `${100 * (Number(b) / Number(a))}%`)
   }
 ];
