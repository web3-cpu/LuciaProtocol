...
export default class DataProxy {
  ...
  getTextFormat(text) {
    ...
    //  Check validation, if valid number, then change it!
    newText = newText.replaceAll(" ", "");
    newText = newText.replaceAll(",", "");

    if (newFormat && /^(-?\d*.?\d*)$/.test(newText)) {
      const chunks = newText.split(".");
      if (newFormat === "usd") {
        if (chunks.length === 1) newFormat += "NoDecimal";
        else {
          if(chunks[1].length < 5) newFormat += "";
          else newFormat += "BigDecimal";
        }
      }
  ...
  setCellText(ri, ci, text, state) {
    const { rows, history, validations, styles } = this;
    if (state === 'finished') {
      rows.setCellText(ri, ci, '');
      history.add(this.getData());

      //  Get the format Keyword from text
      const { newFormat, newText } = this.getTextFormat(text);
      //  change the text
      rows.setCellText(ri, ci, newText);
      //  change attribute
      if (newFormat) this.setSelectedCellAttr("format", newFormat);
    } else {
